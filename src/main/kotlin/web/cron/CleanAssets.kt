...
class CleanAssets : Runnable {
    ...
    companion object {
        ...
        private val proxyfile = FileUtils.createFolderIfNotExist(appCtx.externalFiles, "assets","proxy")
    ...
    override fun run() = runBlocking{
            ...
            codesfile.walk().maxDepth(1).forEach {
                if(it.isFile){
                    kotlin.runCatching {
                        ...
                        if(time > 1){
                            logger.info("clean code ${it.path}")
                            it.delete()
                        }
                    }
                }
        ...
        kotlin.runCatching {
            proxyfile.walk().maxDepth(1).forEach {
                if(it.isFile){
                    kotlin.runCatching {
                        val attributes = Files.readAttributes(it.toPath(), BasicFileAttributes::class.java)
                        val creationTime = attributes.creationTime()
                        ...
                        val time=(System.currentTimeMillis()-instant.toEpochMilli())/(60*60*24*1000)
                        if(time > 30){
                            logger.info("clean proxy ${it.path}")
                            it.delete()
                        }
                    }
                }
